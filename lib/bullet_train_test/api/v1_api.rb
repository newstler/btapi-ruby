  ...
  class V1Api
    ...
    def post_team_project_tags_with_http_info(team_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: V1Api.post_team_project_tags ...'
      end
      # verify the required parameter 'team_id' is set
      if @api_client.config.client_side_validation && team_id.nil?
        fail ArgumentError, "Missing the required parameter 'team_id' when calling V1Api.post_team_project_tags"
      end
      # resource path
      ...
      new_options = opts.merge(
        :operation => :"V1Api.post_team_project_tags",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: V1Api#post_team_project_tags\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
