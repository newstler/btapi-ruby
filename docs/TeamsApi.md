...
| [**post_team**](TeamsApi.md#post_team) | **POST** /teams |  |
...
> <ApiV1TeamSerializer> post_team(opts)
...
begin
  ...
  result = api_instance.post_team(opts)
  p result
rescue BulletTrainTest::ApiError => e
...
> <Array(<ApiV1TeamSerializer>, Integer, Hash)> post_team_with_http_info(opts)

```ruby
begin
  
  data, status_code, headers = api_instance.post_team_with_http_info(opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <ApiV1TeamSerializer>
rescue BulletTrainTest::ApiError => e
