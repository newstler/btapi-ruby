...
api_instance = BulletTrainTest::StoreApi.new
...
api_instance = BulletTrainTest::StoreApi.new
body = BulletTrainTest::Order.new # Order | order placed for purchasing the pet
...
> <Array(<Order>, Integer, Hash)> place_order_with_http_info(body)

```ruby
begin
  # Place an order for a pet
  data, status_code, headers = api_instance.place_order_with_http_info(body)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <Order>
rescue BulletTrainTest::ApiError => e
